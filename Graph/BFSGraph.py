...
def bfs_startegy_for_adjacencyList_levelcalculation(fvAList, fV):
    ...
    while(not tvQueue.isEmpty()):
        tvCurrProcessing = tvQueue.delq()
        tvNeighboursList = CommonGraph.neighboursAList(fvAList, tvCurrProcessing)
        for k in tvNeighboursList:
            if(tvLeval[k] == -1) :
...
tvAdjecenyList  = CommonGraph.create_graph_into_adjacency_list(1)
tvStartIndex = 0
tvOut = bfs_startegy_for_adjacencyList(tvAdjecenyList, tvStartIndex)
print(tvOut.__str__())
...
# tvOut = bfs_startegy_for_adjacencyList_levelcalculation(tvAdjecenyList, 7)
# print(tvOut.__str__())
